...
static int extract_value(const char *line, const char *key_with_eq, char *out, size_t outsz){
    ...
    const char *cands[] = {" ID", " NAME", " PROGRAMME", " MARK", "ID", "NAME", "PROGRAMME", "MARK", NULL};
    for (int i=0; cands[i]; ++i){
        ...
        for (;;){
            ...
            // accept only if followed by optional spaces and '=' (or spaces then key letters)
            const char *t2 = t;
            while (*t2 && isspace((unsigned char)*t2)) t2++;
            if (*t2 == '=' || ci_starts_with(t2, "ID") || ci_starts_with(t2, "NAME")
                || ci_starts_with(t2, "PROGRAMME") || ci_starts_with(t2, "MARK")){
    ...
    int i=0; while (out[i] && isspace((unsigned char)out[i])) i++;
    if (i>0) memmove(out, out+i, n - i + 1);
